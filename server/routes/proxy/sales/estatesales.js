...
function formatData(data, key) {
  return {
    ...
    title: data.orgName || '',
    date:
      data.hasOwnProperty('firstLocalStartDate') &&
      data.firstLocalStartDate.hasOwnProperty('_value')
    ...
    link: `https://www.estatesales.net/${data.stateCode}/${data.cityName}/${
...
module.exports.getEstateSales = function(request, res) {
  ...
  Promise.all(query.map(tag => makeRequest(request, tag)))
      ...
      if (!results || !results.length) {
        res
          ...
          .send({
            error: true,
            code: results[0].code,
            message: 'ERROR: Problem getting Estate Sales results.'
          });
      } else {
    ...
    .catch(err => {
      res
        .status(500)
        .send({ error: true, code: err, message: 'ERROR: Problem getting Estate Sales results.' });
    });
};
